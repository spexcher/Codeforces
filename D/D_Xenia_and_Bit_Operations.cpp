...
void solve()
{
    ...
    // print(v);
    vi segtree(4 * size);
    function<void(int, int, int, int)> build = [&](int index, int l, int r, int parity)
    {
        debug(l);
        debug(r);
        debug(index);
        ...
        int mid = (l + r) / 2;
        build(2 * index + 1, l, mid, parity ^ 1);
        build(2 * index + 2, mid + 1, r, parity ^ 1);
        if (parity == 0)
            segtree[index] = segtree[2 * index + 1] | segtree[2 * index + 2];
        else
            segtree[index] = segtree[2 * index + 1] ^ segtree[2 * index + 2];
    };
    function<void(int, int, int, int, int, int)> update = [&](int index, int l, int r, int parity, int pos, int val)
    {
        debug(l);
        debug(r);
        debug(index);
        ...
        int mid = (l + r) / 2;
        if (pos <= mid)
            update(2 * index + 1, l, mid, parity ^ 1, pos, val);
        else
            update(2 * index + 2, mid + 1, r, parity ^ 1, pos, val);
        if (parity == 0)
            segtree[index] = segtree[2 * index + 1] | segtree[2 * index + 2];
        else
            segtree[index] = segtree[2 * index + 1] ^ segtree[2 * index + 2];
    ...
    while (q--)
    {
        ...
        --a;
        if (n & 1)
            update(0, 0, size - 1, 0, a, b);
        else
            update(0, 0, size - 1, 1, a, b);
        print(segtree[0]);
    }
}