...
void solve()
{
   ...
   // print(v);
    int alice=0;
    int bob=n-1;
    int ca=0,cb=0,move=0,tcb=0,tca=0;
    while(alice<=bob){
        ca=0;
        while(ca<=cb and alice<=bob)ca+=v[alice],tca+=v[alice],alice++;
        move++;
        if(alice>bob)break;
        cb=0;
        while(cb<=ca and bob>=alice)cb+=v[bob],tcb+=v[bob],bob--;
        move++;
    }
    //print(alice<<sp<<bob);
    print(move<<sp<<tca<<sp<<tcb);
}