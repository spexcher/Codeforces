...
void solve()
{
    ...
    vi v(n);
    cin >> v;
    FoF(i, 0, n - 1) v.pb(v[i]);
    vi pre(all(v));
    FoF(i, 1, 2 * n - 1) pre[i] += pre[i - 1];
    auto prefixsum = [&](int i, int j)
    {
        if (i == 0)
            return pre[j];
        else
    ...
    while (q--)
    {
        ...
        if (rr != rl)
        {
            sum += (prefixsum(l + rl - 1, n + rl - 2));
            sum += (prefixsum(rr - 1, r + rr - 1));
        }
        else
            sum += prefixsum(l + rl - 1, r + rl - 1);
        print(sum);
    }
}